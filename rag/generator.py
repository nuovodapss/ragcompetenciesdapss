...
def answer_question(
    ...
    if mode == "sintesi_locale_light" and llm is not None:
        ...
        try:
            ...
            warning = f"Sintesi locale non disponibile in runtime: {exc}. Ho usato la modalità estrattiva."
    else:
        answer = build_extractive_answer(question, results)
        if mode == "sintesi_locale_light" and llm is None:
            warning = (
                f"Sintesi locale non caricata. Motivo: {llm_error}. Ho usato la modalità estrattiva."
                if llm_error
